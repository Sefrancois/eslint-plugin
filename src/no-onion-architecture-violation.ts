...
export class NoOnionArchitectureViolation {
	private static MESSAGE = "This import violates Onion Architecture principles. Read more about this architecture : " +
	...
	private static APPLICATION_SERVICE_LAYER = new RegExp(/(applications?-services?)|(applications?)/);
	...
	public static checkNoCrossDomainImport(context: Rule.RuleContext, node: ImportDeclaration & Rule.NodeParentExtension): void {
		...
		if (this.isImportFromLayer(currentImportPath, this.INFRASTRUCTURE_LAYER) && this.isFileFromLayers(currentFilePath, this.APPLICATION_SERVICE_LAYER, this.DOMAIN_SERVICE_LAYER, this.DOMAIN_MODEL_LAYER)) {
			context.report({ node, message: this.MESSAGE });
		} else if (this.isImportFromLayer(currentImportPath, this.APPLICATION_SERVICE_LAYER) && this.isFileFromLayers(currentFilePath, this.DOMAIN_SERVICE_LAYER, this.DOMAIN_MODEL_LAYER)) {
			context.report({ node, message: this.MESSAGE });
		} else if (this.isImportFromLayer(currentImportPath, this.DOMAIN_SERVICE_LAYER) && this.isFileFromLayers(currentFilePath, this.DOMAIN_MODEL_LAYER)) {
			context.report({ node, message: this.MESSAGE });
		}
	}
